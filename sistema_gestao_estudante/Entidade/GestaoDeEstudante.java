...
public class GestaoDeEstudante extends JFrame implements ActionListener,MouseListener {
	...
	public GestaoDeEstudante() {
		...
		*/
		...
		Nome.setBounds(22, 21, 46, 14);
		panel.add(Nome);
		
		JLabel lblTeste = new JLabel("sobrenome");
		lblTeste.setBounds(22, 46, 70, 14);
		panel.add(lblTeste);
		
		JLabel lblTeste_1 = new JLabel("telefone");
		lblTeste_1.setBounds(22, 71, 46, 14);
		panel.add(lblTeste_1);
		
		JLabel lblTeste_y= new JLabel("sexo");
		lblTeste_y.setBounds(22, 96, 86, 14);
		panel.add(lblTeste_y);
	
		/*txtCodigo = new JTextField();
		txtCodigo.setBounds(190, 18, 20, 20);
		panel.add(txtCodigo);
		txtCodigo.setColumns(10);*/
		
		txtNome = new JTextField();
		txtNome.setColumns(10);
		txtNome.setBounds(78, 18,86, 20);
		panel.add(txtNome);
		
		txtSobrenome = new JTextField();
		txtSobrenome.setColumns(10);
		txtSobrenome.setBounds(78, 43,86, 20);
		panel.add(txtSobrenome);
		
		txtTelefone = new JTextField();
		txtTelefone.setColumns(10);
		txtTelefone.setBounds(78, 68,86, 20);
		panel.add(txtTelefone);
		
		txtSexo = new JTextField();
		txtSexo.setColumns(10);
		txtSexo.setBounds(78, 93, 86, 20);
		panel.add(txtSexo);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==btnAdicionar){
			...
			try {
				...
				JOptionPane.showMessageDialog(null, e1.toString()+" -->56Gestao");
		...
		if(e.getSource()==btnActualizar){
			...
			try {
				ControllerEstudante.actualizar( nome, sobrenome,telefone, sexo );
				JOptionPane.showMessageDialog(null, "atualizado com sucesso");
				limparCaixas();
				limparTabela();
				listarEstudante();
			} catch (SQLException e1) {
				JOptionPane.showMessageDialog(null, e1.toString()+" ->8Gestao");
			}
		}
		if(e.getSource()==btnRemover){
			...
			try {
				...
				JOptionPane.showMessageDialog(null, e1.toString()+" ->2Gestao");
			}
		}
